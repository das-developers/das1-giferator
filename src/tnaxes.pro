...
;	L. Granroth, 2003-03-14 -- multi-year date labels
;	  Since there's no local info on date, the first major tick
;	  aligns with the first day boundary in the plot.
...
;	IF XINC LT 3.5  THEN begin XI = 2.5  & xi2 = 0.5  & endif
	...
	;--------  Multi-year date labels --------
	; Note that year-sized intervals (365.25 days) align with
	; the first day of the interval, not necessarily the beginning
	; of a year.  Also, when the interval does begin on a year
	; boundary, major ticks may occur during Dec 31 of the preceding
	; year.
	if xi gt 86400.0 * 200.0 then begin
	  FORM = 'Y$n$0d$'
	  if xi gt 86400.0 * 600.0 then begin
	    if xi gt 86400.0 * 1500.0 then begin
	      if xi gt 86400.0 * 3000.0 then begin
	        xi = 86400.0 * 365.25 * 10.0
	        xi2 = 86400.0 * 365.25 * 2.0
	      endif else begin
	        xi = 86400.0 * 365.25 * 5.0
	        xi2 = 86400.0 * 365.25
	      endelse
	    endif else begin
	      xi = 86400.0 * 365.25 * 2.0
	      xi2 = 86400.0 * 365.25
	    endelse
	  endif else begin
	    xi = 86400.0 * 365.25
